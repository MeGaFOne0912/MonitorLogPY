...
def maiorMenorTempo(logs):
    ...
    for linha in logs:
        if ' ms - ' in linha:
            ...
            posicao_espaco = linha.rfind(' ', 0, posicao_ms - 1)
            trecho_numero = ''
            indice = posicao_espaco + 1
            while indice < posicao_ms:
                trecho_numero += linha.__getitem__(indice)
                indice += 1
            if trecho_numero.isdigit():
                tempo = int(trecho_numero)
                if maior is 0 or tempo > maior:
                    maior = tempo
                if menor is 0 or tempo < menor:
                    menor = tempo
    return maior, menor
    ...
    for linha in logs:
        if ' ms - ' in linha:
            ...
            while indice < posicao_ms:
                trecho_numero += linha.__getitem__(indice)
                indice += 1
            trecho_numero = trecho_numero.strip()
            if trecho_numero.isdigit():
                tempo = int(trecho_numero)
...
def distribuicaoDeStatus(logs):
    for linha in logs:
        if ' - ' in linha:
            ...
            status = ''
            indice = inicio_status
            while indice < fim_status:
                status += linha.__getitem__(indice)
                indice += 1
                status = status.strip()
